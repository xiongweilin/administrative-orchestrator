...
def main() -> int:
    ...
    try:
        ...
        state_path = _validate_kernel_state_path()
    ...
    print(
        ...
        f"kernel_revision={settings.kernel_supported_revision} "
        f"kernel_state={state_path} auth=oidc hris=odoo iam=keycloak"
    )
    return 0
...
def _validate_kernel_state_path() -> Path:
    ...
    if not parent.exists() or not parent.is_dir():
        raise ProductionReadinessError("production Kernel state parent directory does not exist")
    if not os.access(parent, os.W_OK):
        raise ProductionReadinessError("production Kernel state parent directory is not writable")
    return path
