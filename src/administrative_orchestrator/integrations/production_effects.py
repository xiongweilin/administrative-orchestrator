    ...
    async def observe(
        ...
        *,
        ...
        try:
            ...
            # Feishu returns a single-message GET as ``data.items[0]`` in the
            # current provider API.  Keep accepting the older direct-message
            # shape as well; both are canonical provider readback, not local
            # delivery claims.
...
class OdooFinancialEffectConnector:
    ...
    async def invoke(
        ...
        *,
        ...
        try:
            ...
            # An employee is the governed subject of an expense, but it is
            # not a transaction identity: one employee may submit many
            # expenses. Request identity remains the only replay key there.
    ...
    async def find_existing_vendor_bills(
        ...
        *,
        ...
        """Read the authoritative ERP identity used by duplicate qualification."""
    ...
    def _create_values(
        ...
        *,
        ...
        # ``subject_ref`` is a Kernel routing/control parameter added to the
        # frozen effect intent.  It is already persisted in its dedicated
        # identity field and is not part of the business payload whose exact
        # equality is independently verified by M8.
            ...
            values.update(
                {
                    ...
                    # Odoo requires a quantity for hr.expense. M8 models a
                    # prepared expense as one bounded receipt-backed record;
                    # the exact amount remains the record's unit/total value.
                    "quantity": 1,
        ...
        *,
        ...
        try:
            ...
            if self.operation == "purchase_order.confirm" and state not in {"purchase", "done"}:
                # A subject match alone is insufficient for an irreversible
                # confirmation. Preserve the observed state so an unchanged
                # draft cannot be treated as a successful confirmation.
