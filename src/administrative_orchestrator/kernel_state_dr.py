...
class KernelStateRecoveryError(RuntimeError):
    pass


def backup_kernel_state(source: Path, destination: Path) -> str:
    """Create a consistent online SQLite backup and sidecar SHA-256 manifest."""

    source = source.expanduser().resolve()
    destination = destination.expanduser().resolve()
    if source == destination:
        raise KernelStateRecoveryError("Kernel state backup destination must differ from source")
    if not source.is_file():
        raise KernelStateRecoveryError(f"Kernel state source does not exist: {source}")
...
def verify_kernel_state_backup(backup: Path) -> str:
    backup = backup.expanduser().resolve()
    if not backup.is_file():
        raise KernelStateRecoveryError(f"Kernel state backup does not exist: {backup}")
    _quick_check(backup)
    ...
    if manifest.exists():
        ...
        if not expected or expected != digest:
            raise KernelStateRecoveryError("Kernel state backup digest does not match manifest")
    return digest


def restore_kernel_state(backup: Path, destination: Path, *, force: bool = False) -> str:
...
def _quick_check(path: Path) -> None:
    ...
    if rows != ["ok"]:
        raise KernelStateRecoveryError("Kernel state backup failed SQLite quick_check")
...
def _require_parent(path: Path) -> None:
    if not path.parent.exists() or not path.parent.is_dir():
        raise KernelStateRecoveryError(f"destination parent does not exist: {path.parent}")
