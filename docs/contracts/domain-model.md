...
Administrative turns observed organizational input into a qualified administrative matter, closes the applicable policy and authority questions, derives explicit business obligations, delegates bounded physical execution to Agent Kernel, independently verifies resulting reality, and closes or reopens the matter without collapsing uncertainty into success.
...
A meeting commitment is a second entry shape into the same responsibility model: a qualified commitment becomes durable administrative state and persistent Kernel responsibility, but commitment creation itself does not imply external Work or an effect.
...
A proposed administrative request inferred from source material. It is not yet an `AdministrativeRequest` and cannot directly create a case, authority, Kernel Work, or external effect.
...
Admission means only that a candidate is qualified to enter the formal Administrative model. Admission is not proof that every candidate fact is true, and it is unrelated to Agent Kernel Work admission.

```text
Candidate != AdministrativeRequest
model confidence != admission authority
admission != fact authority
Administrative admission != Kernel Work admission
```
...
`ExecutionAuthorization` is the canonical Administrative term. It is distinct from Agent Kernel runtime authorization, which is owned by Kernel.
...
A qualification may be a required input to governance, but qualification alone does not create a Decision, `ApprovalSatisfaction`, `ExecutionAuthorization`, Kernel Work, or external effect.
...
Physical provider execution, retry permission, runtime authorization, and generic recovery belong to Agent Kernel after cut-over.
...
A bounded Administrative semantic result derived from verified realization evidence. A provider response, transport acceptance, Kernel execution receipt, or object existence is not by itself a `ConfirmedOutcome`.
...
Persistent responsibility is Kernel-owned. Administrative may cause or reference Kernel responsibility for obligations or commitments, and may provide completion evidence to Kernel's assessment/decision/transition protocol, but Administrative does not redefine Kernel responsibility state.
...
The formal admitted self-commitment. It binds the committer Principal, exact action, offset-aware due time and its basis, case/authority epoch, optional scope/beneficiary, and Kernel responsibility references.
