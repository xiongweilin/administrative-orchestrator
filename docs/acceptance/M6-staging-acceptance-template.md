...
| --- | --- | --- | --- |
| A — Feishu authenticity | The official Feishu SDK long connection is active and a real event is received; the gateway forwards only trusted metadata and authenticates the handoff with the dedicated gateway transport credential. Administrative accepts the authenticated envelope, rejects a missing/wrong transport credential, rejects a conflicting provider token when present, and a forged metadata envelope creates no receipt. URL verification and signed HTTP callback headers are `N/A` for this topology unless that transport is separately enabled. | `[ ]` | `[reference]` |
