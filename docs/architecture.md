...
- **M7 — employee lifecycle responsibility (in progress):** employee-offboarding as a second case kind, authoritative termination facts, qualified effective-time waiting, Administrative authority revocation, continuity transfer with explicit successor requirements, Kernel-owned IAM/HRIS revocation effects, verified completion, and explicit persistent-responsibility discharge. The contract is ADR 0004 with the plan in `docs/milestones/M7.md`.
