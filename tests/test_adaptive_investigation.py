...
class _ReconciliationVerifier:
    def verify(self, case_id, authority_epoch, reconciliation_ref) -> None:
        ...
        if reconciliation_ref != "kernel:reconciliation:completed":
            raise KernelReconciliationVerificationError("unverified Kernel reference")


class _KernelProjectionRepository:
    def list_projections_for_case(self, case_id):
        return [
            SimpleNamespace(
                case_id=case_id,
                authority_epoch=1,
...
def test_outcome_unknown_requires_kernel_reconciliation_reference() -> None:
    store = _store()
    case = _case(store)
    service = InvestigationService(store)
...
def test_outcome_unknown_requires_verified_kernel_reconciliation() -> None:
    store = _store()
    case = _case(store)
    service = InvestigationService(store)
    with pytest.raises(InvestigationConflict, match="verifier is unavailable"):
        service.request_investigation(
            ...
            reconciliation_ref="not-a-kernel-record",
        )
    ...
    request = verified.request_investigation(
        case.case_id,
        trigger_type=InvestigationTriggerType.OUTCOME_UNKNOWN,
        reason="Kernel reconciliation completed with residual ambiguity",
        requested_question="is there residual Administrative ambiguity?",
        created_by="person:operator",
        idempotency_key="outcome-unknown-verified",
        reconciliation_ref="kernel:reconciliation:completed",
    )
    assert request.trigger.reconciliation_ref == "kernel:reconciliation:completed"
...
def test_kernel_reconciliation_reference_is_bound_to_projection_and_resolution() -> None:
    verifier = LocalKernelReconciliationVerifier(
        repository=_KernelProjectionRepository(),
        recovery_client=_KernelResolutionClient(),
    )
    case_id = uuid4()

    verifier.verify(case_id, 1, "kernel:execution:unknown")
    with pytest.raises(KernelReconciliationVerificationError, match="not bound"):
        verifier.verify(case_id, 1, "not-a-kernel-record")
