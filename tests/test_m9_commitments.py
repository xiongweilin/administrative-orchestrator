...
from administrative_orchestrator.integrations.kernel.client import KernelSubmissionError
...
def test_kernel_commitment_responsibility_provisioner_records_prefix_and_fails_closed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    class Client:
        ...
        def submit_payload(self, **payload):
            assert payload["responsibility_payload"]["object_type"] == "StandingResponsibility"
            assert payload["proposal_payload"]["requested_resources"]["money_minor"] == 0
            if self.should_fail:
                raise KernelSubmissionError("kernel unavailable")
            return SimpleNamespace(
                responsibility_ref="resp:m9",
                admission_ref="admission:m9",
                assessment_ref="assessment:m9",
                proposal_ref="proposal:m9",
            )
    ...
    class Bridge:
        ...
        def compatibility(self):
            return SimpleNamespace(persistent_responsibility_contract="responsibility-v1")

        def client(self):
            return client

    monkeypatch.setattr(commitment_service_module, "KernelExecutionBridge", Bridge)
    provisioner = KernelCommitmentResponsibilityProvisioner(
        store,
        settings=Settings(database_url="sqlite+pysqlite:///:memory:", kernel_bridge_mode="disabled"),
    )
    refs = provisioner.provision(case=case, commitment=commitment, governance_basis=governance)
    assert refs.responsibility_ref == "resp:m9"
    ...
    with pytest.raises(CommitmentIntakeError, match="was not recorded"):
        provisioner.provision(case=case, commitment=commitment, governance_basis=governance)
