...
def _settings(*, external: bool, kernel: str = "disabled"):
    return SimpleNamespace(
        worker_database_url=None,
        database_url="sqlite://",
        sandbox_base_url="http://sandbox",
        provider_timeout_seconds=1.0,
        kernel_bridge_mode=kernel,
...
def test_drive_offboarding_selects_production_trust_and_kernel_cutover(
    monkeypatch,
) -> None:
    case = _case(CaseStatus.AUTHORIZED)
    ...
    monkeypatch.setattr(
        definitions, "get_settings", lambda: _settings(external=True, kernel="cutover")
    )
    monkeypatch.setattr(definitions, "SqlStore", lambda url: store)
    monkeypatch.setattr(definitions, "HttpEffectProvider", lambda *args, **kwargs: provider)
    monkeypatch.setattr(definitions, "KernelExecutionBridge", _Bridge)
    monkeypatch.setattr(
        definitions,
