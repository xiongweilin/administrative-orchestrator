...
def test_readme_separates_current_language_from_acceptance_history() -> None:
    text = (ROOT / "README.md").read_text(encoding="utf-8")
    assert "docs/contracts/domain-model.md" in text
    assert "## Historical acceptance" in text
    assert "compatibility identifiers" in text.lower()
